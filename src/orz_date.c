...
static uint32_t __date_mdays(uint32_t year, uint32_t month)
{
    return __calc_mdays(orz_date_isleap(year), month);
}

static uint32_t __date_accum_days(uint32_t year, uint32_t month)
{
...
static uint32_t __date_calc_month(uint32_t year, uint32_t days)
{
...
static uint32_t __date_week(uint32_t year, uint32_t month, uint32_t day)
{
    if (month <= 2) {
        year -= 1;
        month += 12;
...
static int32_t __leaps(int32_t year)
{
...
static int __calc_year_and_yday(uint32_t year, uint32_t month, uint32_t day, uint32_t offset)
{
...
static int __date_calc(uint32_t *y, uint32_t *m, uint32_t *d, int offset)
{
    int ret = __calc_year_and_yday(*y, *m, *d, offset);
    int yday = ret & 0x1FF;
    uint32_t year = ret >> 9;
